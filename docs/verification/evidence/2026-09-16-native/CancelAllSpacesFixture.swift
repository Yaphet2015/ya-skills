import AppKit
let root = "/tmp/ya-native-20260916/matrix-cancel-allspaces"
...
final class BlockingButton: NSButton {
 override func accessibilityPerformPress() -> Bool {
  let data: [String: Any] = ["at": Date().timeIntervalSince1970, "pid": ProcessInfo.processInfo.processIdentifier, "kind": "AXPress entered"]
  if let json = try? JSONSerialization.data(withJSONObject: data) { try? json.write(to: URL(fileURLWithPath: root + "/entered.json"), options: .atomic) }
  let deadline = Date().addingTimeInterval(15)
  while Date() < deadline && !FileManager.default.fileExists(atPath: root + "/release") { Thread.sleep(forTimeInterval: 0.01) }
  clicks += 1; record("blocking-button-completed")
  return true
 }
}
...
let button = BlockingButton(title: "Block Increment", target: handler, action: #selector(Handler.clicked(_:)))
