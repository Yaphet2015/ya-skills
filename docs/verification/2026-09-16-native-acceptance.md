...
用户明确报告：上述测试期间，鼠标和键盘出现强占行为。因此，前台 PID 不变、fixture 的 key/main window 为 false，仅证明所记录的窗口状态；**不能证明用户输入未受干扰**。此前坐标命中、取消、去重等功能结果保留，但“不干扰用户鼠标、键盘”验收未通过。

当前桌面暂停原生输入测试，只执行源码检查、无桌面回归和只读权限／安装验证。新的真实输入验收需要独立测试桌面，或能够在派发前禁止合成事件及焦点变更的已验证路径。SDK 0.27 的 `type_text` 工具契约允许 AX 失败后自动回退到 CGEvent，`Background` 本身不足以满足这一约束。
后续代码增加了 `Computer.setValue(target, elementToken, value)` 严格 AX-only seam：它只调用 generic `set_value`，并要求结构化结果同时报告 `route: "accessibility"` 与 `effect: "confirmed"`；缺失或其他 route 会按 unknown 停止。该代码路径尚未在独立桌面表单上实跑，因此不能替代完整原生验收。
